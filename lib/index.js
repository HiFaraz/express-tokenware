			...
			try {
				jsonwebtoken.verify(req.headers["authorization"].split(" ")[1], secretOrPrivateKey, options.verify, function (err, decodedBearerToken) {
					if (err) {
						...
						var tokenIsRevoked = isRevokedToken(req.headers["authorization"].split(" ")[1]);
						if (tokenIsRevoked && !options.allowAnonymous) next(_error('revokedToken', 'Request authorization was previously revoked'));
						else {
							req.isAnonymous = tokenIsRevoked;
							if (!tokenIsRevoked) req.decodedBearerToken = decodedBearerToken;
							next();
						}
					}
				});
