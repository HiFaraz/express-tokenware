...
module.exports = function (secretOrPrivateKey, options, isRevokedToken) {
	...
	var _exports = function (req, res, next) {
		var _errorHandler = function (errorToHandle) {
			...
			var _responseTable = {
				...
				payloadMissing: [httpCodes.INTERNAL_SERVER_ERROR, errorToHandle, true],
				revokedToken: httpCodes.UNAUTHORIZED,
				signedBearerTokenMissing: [httpCodes.INTERNAL_SERVER_ERROR, errorToHandle, true],
		...
		var _authenticate = req.bearerTokenPayload;
		var _authorize = typeof authorizationHeader !== 'undefined' && bearerTokenPrefix == 'Bearer'
		typeof bearerToken !== 'undefined';

		if (_authenticate) {
			_report('payload found');
			req.signedBearerToken = jsonwebtoken.sign(req.bearerTokenPayload, secretOrPrivateKey, options.sign);
		...
		if (_authorize) {
			_report('token found in header');
			try {
				jsonwebtoken.verify(bearerToken, secretOrPrivateKey, options.verify, function (error, decodedBearerToken) {
					if (error) {
						_rejectToken(error);
					} else {
				...
				_rejectToken(_error('unknown', 'Could not verify token, likely that "iat" claim is missing from the token'));
			}
		}
		...
		(req.tokenwareError) ? next(req.error): next();
	};
