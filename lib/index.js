...
module.exports = function (secretOrPrivateKey, options, isRevokedToken) {
	...
	var rejectVerification = function (error, req, next) {
	...
	_exports.verify = function (req, res, next) {
		...
		if (typeof authorizationHeader == 'undefined') {
			rejectVerification(_error('noAuthorizationHeader', 'Request is missing authorization header'), req, next);
		} else if (bearerTokenPrefix !== 'Bearer' || typeof bearerToken == 'undefined') {
			rejectVerification(_error('malformedAuthorizationHeader', 'Authorization header is malformed, should be in the form of: Bearer <token>'), req, next);
		} else {
			try {
				jsonwebtoken.verify(bearerToken, secretOrPrivateKey, options.verify, function (err, decodedBearerToken) {
					if (err) {
						rejectVerification(err, req, next);
					} else {
						if (isRevokedToken(bearerToken)) {
							rejectVerification(_error('revokedToken', 'Request authorization was previously revoked'), req, next);
						} else {
				...
				rejectVerification(_error('unknown', 'Could not verify token, likely that "iat" claim is missing from the token'), req, next);
			}
		}
	};
